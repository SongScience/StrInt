...
def align_lr_gene(self):
    ...
    lr_df_ci = lr_df.copy()
    lr_df_ci[0] = lr_df_ci[0].astype(str).str.lower().map(lower_map)
    lr_df_ci[1] = lr_df_ci[1].astype(str).str.lower().map(lower_map)
    lr_df_ci = lr_df_ci.dropna()
    lr_df_ci = lr_df_ci[lr_df_ci[0].isin(genes) & lr_df_ci[1].isin(genes)]
    lr_df_ci = lr_df_ci.drop_duplicates()
    return lr_df_ci
...
def find_executable_in_path(executable_name):
    """Find the full path of an executable from PATH or the active env."""
    executable_path = shutil.which(executable_name)
    if executable_path:
        return executable_path

    # Fallback for Jupyter sessions where PATH was not propagated correctly.
    candidate_dirs = []
