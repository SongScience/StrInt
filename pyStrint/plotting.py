...
def draw_lr_flow3(df, left_panel = 'ligand', mid_panel = 'receptor', right_panel = 'pathway',
    ...
    import os
    import plotly.graph_objects as go
    # Some environments start Kaleido with a restricted PATH, which breaks
    # helper commands like `dirname` used by the kaleido launcher script.
    default_path = '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'
    os.environ['PATH'] = f"{os.environ.get('PATH', '')}:{default_path}".strip(':')
    ligs = list(df[left_panel].unique())
...
def LRI_of_CCI(adata_orig, sender = '',receiver = '',ligand = '', receptor = '',
                ...
                bg_alpha = 0.4, cell_alpha = 0.7, arrow_alpha = 0.8):
    ...
    sns.scatterplot(data = draw_bg, x = cols[0], y=cols[1], 
                    s = 10, alpha = bg_alpha,c=['#ccc'],edgecolor = None)
    
    sns.scatterplot(data = celltype_df, x=cols[0], y=cols[1],hue = hue, 
                    s = size,alpha = cell_alpha, palette=color_map, edgecolor = None)      
    ...
    for _,row in target_cellpair_arrow.iterrows():
        ...
        plt.arrow(start[0], start[1], dx, dy,
          ...
          overhang = 0.2, alpha = arrow_alpha,
    ...
    if savefig:
        if isinstance(savefig, str):
            ...
            save_path = adata.uns['figpath']
            out_path = f'{save_path}/{sender}_{receiver}_{ligand}_{receiver}_arrow.pdf'
        ...
        if savefig:
            if isinstance(savefig, str):
                if savefig.endswith('.pdf'):
                    out_path = savefig[:-4] + '_region.pdf'
                else:
                    out_path = savefig + '_region.pdf'
            else:
                save_path = adata.uns['figpath']
                out_path = f'{save_path}/{sender}_{receiver}_{ligand}_{receiver}_region.pdf'
            plt.savefig(out_path, dpi=300, bbox_inches='tight')
            print(f'Figure saved {out_path}')
        plt.show()
...
def gsea_enrichment(gsea_res, k=1, figsize_heat=(6, 5), figsize_gsea=(4, 4)):
    ...
    print(res_df)
    genes = res_df.Lead_genes.iloc[k].split(";")
    print(genes)
    ax = gp.heatmap(df=gsea_res.heatmat.loc[genes],
                    z_score=None,
                    title=res_df.Term.iloc[k].split(' Hom')[0],
    ...
    axs = gsea_res.plot(terms=term.iloc[k], figsize=figsize_gsea)
