...
def _resolve_threshold(series, value):
    q = _parse_quantile(value)
    if q is not None:
        return float(series.quantile(q))
    return value


def _filter_low_detected_genes(sc_exp, st_exp, min_cells=3, min_counts=10):
    """
    Filter genes by detection and total counts across both sc and st datasets.

    A gene is kept if it is expressed in at least `min_cells` cells/spots in
    BOTH sc and st, and has at least `min_counts` total counts in BOTH sc and st.
    """
    if min_cells is None and min_counts is None:
        return sc_exp, st_exp

    sc_detect = (sc_exp > 0).sum(axis=0)
    st_detect = (st_exp > 0).sum(axis=0)
    sc_total = sc_exp.sum(axis=0)
    st_total = st_exp.sum(axis=0)

    min_cells_sc = _resolve_threshold(sc_detect, min_cells)
    min_cells_st = _resolve_threshold(st_detect, min_cells)
    min_counts_sc = _resolve_threshold(sc_total, min_counts)
    min_counts_st = _resolve_threshold(st_total, min_counts)

    sc_keep = pd.Series(True, index=sc_exp.columns)
    st_keep = pd.Series(True, index=st_exp.columns)

    if min_cells is not None:
        sc_keep &= sc_detect >= min_cells_sc
        st_keep &= st_detect >= min_cells_st

    if min_counts is not None:
        sc_keep &= sc_total >= min_counts_sc
        st_keep &= st_total >= min_counts_st

    keep_genes = sc_keep & st_keep
    sc_exp = sc_exp.loc[:, keep_genes]
    st_exp = st_exp.loc[:, keep_genes]
    return sc_exp, st_exp


def filter_pipeline_genes(sc_exp, st_exp, sc_distribution, species, min_cells=3, min_counts=10):
    """
    Filter to the downstream gene set used by the pipeline:
    - shared non-zero genes between sc_exp and st_exp
    - intersect with sc_distribution genes
    - remove MT genes for the given species
    - filter low-detected genes using min_cells/min_counts across sc and st
    Returns filtered sc_exp, st_exp, sc_distribution, and the gene list.
    """
    sc_exp, st_exp = data_clean(sc_exp, st_exp)
    sc_exp, st_exp = _filter_low_detected_genes(sc_exp, st_exp, min_cells=min_cells, min_counts=min_counts)
    genes = denoise_genes(sc_exp, st_exp, sc_distribution, species)
    sc_exp = sc_exp[genes]
    st_exp = st_exp[genes]
    sc_distribution = sc_distribution[genes]
    return sc_exp, st_exp, sc_distribution, genes
...
def lr2kegg(lri_df, use_lig_gene = True, use_rec_gene = True):
    ...
    '''
    pval_col = None
    for candidate in ['lr_co_ratio_pvalue', 'co_exp_p', 'co_exp_ratio_pvalue', 'pvalue', 'p_val', 'pval']:
        if candidate in lri_df.columns:
            pval_col = candidate
            break

    if pval_col is None:
        # Fallback to neutral significance when p-value-like columns are absent.
        lri_df = lri_df.copy()
        lri_df['lr_co_ratio_pvalue'] = 1.0
        pval_col = 'lr_co_ratio_pvalue'

    if use_lig_gene:
        a = lri_df[['ligand','lr_co_exp_num',pval_col]]
        a.columns = ['gene','lr_co_exp_num','lr_co_ratio_pvalue']
    else:
        a = pd.DataFrame(columns = ['gene','lr_co_exp_num','lr_co_ratio_pvalue'])

    if use_rec_gene:
        b = lri_df[['receptor','lr_co_exp_num',pval_col]]
